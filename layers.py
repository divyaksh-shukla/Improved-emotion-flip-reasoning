    ...
    def forward(self, utterances, speakers):
        ...
        for batch_id in range(batch_size):
            ...
            for utterance_id in range(utterance_len):
                ...
                tmp = torch.unsqueeze(torch.unsqueeze(fop2[utterance_id, :].clone(), 0), 0)
...
class MaskedAttention(nn.Module):
    ...
    def forward(self, key, query, mask=None):
        if mask is None:
            mask = torch.zeros(query.size()).to(query.device)
...
class MemoryNetwork(nn.Module):
  def __init__(self, config_key=None):
    ...
    self.masked_attention = MaskedAttention(batch_size, seq_len, hidden_size).to(CONFIG['device'])
  
  def forward(self, global_output, speaker_output):
    memory_output = global_output
    for hop in range(self.num_hops):
      dialogue,h = self.memory_GRU(memory_output)
      memory_output = self.masked_attention(dialogue,speaker_output)
